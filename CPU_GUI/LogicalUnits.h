...
struct SelectPC_ {
	...
	void Proc(int M_icode, int M_Cnd, int M_valA, int W_icode, int W_valM, int F_predPC, int staddr, int M_begin) {
		if (M_icode == IJXX && !M_Cnd) {
			ans = M_valA;
			printf("Warning! The Prediction of Jump is False\n");
			step = 2;
		}
		else if (W_icode == IRET && W_valM != M_begin) {
			ans = W_valM;
			printf("Warning! The Prediction of Return is False\n");
			step = 3;
		}
		else {
			ans = F_predPC;
...
struct Needregids_ {
	...
	void Proc(int f_icode) {
		val = set<int> { IRRMOVL, IOPL, ICMPL, IIROPL, IIRCMPL, IPUSHL, IPOPL, IIRMOVL, IRMMOVL, IMRMOVL }.count(f_icode);
	}
};
...
struct NeedvalC_ {
	...
	int Get() { return val; }
	void Proc(int f_icode) {
		val = set<int>{ IIRMOVL, IRMMOVL, IIROPL, IIRCMPL, IMRMOVL, IJXX, ICALL }.count(f_icode);
	}
};
/*
HCL:
...
struct Instrvalid_ {
	...
	void Proc(int f_icode) {
		val = set<int> { 
			INOP, IHALT, IRRMOVL, IIRMOVL, IRMMOVL, IMRMOVL,
	  	    IOPL, ICMPL, IIROPL, IIRCMPL, IJXX, ICALL, IRET, IPUSHL, IPOPL
		}.count(f_icode);		
	}
};
...
struct PredictPC_ {
	int ans;
	int Get() { return ans; }
	void Proc(int f_icode, int f_valC, int f_valP, int staddr) {
		if (f_icode == IRET && staddr != -1) ans = staddr;
		else if (f_icode == IJXX || f_icode == ICALL) ans = f_valC;
		else ans = f_valP;		
	}
};
...
struct d_dstE_ {
	...
	void Proc(int D_icode, int D_rB) {
		if (set<int>{ IRRMOVL, IIRMOVL, IIROPL, IOPL }.count(D_icode)) val = D_rB;
...
struct d_srcA_ {
	...
	int Get() { return val; }
	void Proc(int D_icode, int D_rA) {
		if (set<int>{ IRRMOVL, IRMMOVL, IOPL, ICMPL, IPUSHL }.count(D_icode)) val = D_rA;
...
struct d_srcB_ {
	...
	int Get() { return val; }
	void Proc(int D_icode, int D_rB) {
		if (set<int>{ IOPL, ICMPL, IIROPL, IIRCMPL, IRMMOVL, IMRMOVL }.count(D_icode)) val = D_rB;
		else if (set<int> { IPUSHL, IPOPL, ICALL, IRET }.count(D_icode)) val = RESP;
		else val = RNONE;
	}
};
...
struct ALUA_ {
	...
	void Proc(int E_icode, int E_valC, int E_valA) {
		if (set<int>{ IRRMOVL, IOPL, ICMPL }.count(E_icode)) val = E_valA;
		else if (set<int>{ IIRMOVL, IRMMOVL, IMRMOVL, IIROPL, IIRCMPL }.count(E_icode)) val = E_valC;
...
struct ALUB_ {
	...
	void Proc(int E_icode, int E_valB) {
		if (set<int>{ IRMMOVL, IMRMOVL, IIROPL, IIRCMPL, IOPL, ICMPL, ICALL, 
...
struct ALUfun_ {
	...
	void Proc(int E_icode, int E_ifun) {
		if (set<int> { IOPL, IIROPL }.count(E_icode))
			val = E_ifun;
		else if (set<int> { ICMPL, IIRCMPL }.count(E_icode))
			val = ALUSUB;
		else val = ALUADD;
	}
};
...
struct e_dstE_ {
	...
	void Proc(int E_icode, int e_Cnd, int E_dstE) {
		val = E_dstE;
		//if (set<int> { IRRMOVL }.count(E_icode) && !e_Cnd) val = RNONE;
		//else val = E_dstE;
	}
};
...
struct SetCC_ {
	...
	void Proc(int E_icode, int W_stat, int m_stat) {
		val = (set<int> { IOPL, IIROPL, ICMPL, IIRCMPL }.count(E_icode) && 
